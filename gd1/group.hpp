...
namespace gd{
    ...
    //+++++++++++++++++++++//
    //->    group.hpp, group.inl    <-//
    //(    gd::Group<T, D> Lamination Template for Lamination    )//
    ...
    struct Group{
        ...
        // Reference Functions
        template<uint16_t N>
              auto           ref        (void)                     const noexcept -> typename std::enable_if_t<(N<D), const T&>;
        #ifdef GROUPDATA_GROUP_EXCEPTION
        const T&             ref        (uint16_t)                 const;
        #else
        const T&             ref        (uint16_t)                 const noexcept;
        #endif
        const Group<T, D>*   spec       (void)                     const noexcept;
        const Group<T, D>&   operator() (size_t)                   const noexcept;
              ConstRange<T>  crange     (uint16_t =D)              const noexcept;
              ConstRange<T>  crange     (uint16_t, uint16_t)       const noexcept;
        ...
        Group<T, D>& operator[] (size_t)             noexcept;
    ...
    struct ElasticGroup{
        ...
        const T&             ref        (uint16_t)                 const noexcept;
        #endif
        const ElasticGroup<T, D>*   spec       (void)                     const noexcept;
        const ElasticGroup<T, D>&   operator() (size_t)                   const noexcept;
              ConstRange<T>  crange     (uint16_t =D)              const noexcept;
              ConstRange<T>  crange     (uint16_t, uint16_t)       const noexcept;
        ...
        T&           path       (uint16_t)           noexcept;
        #endif     
        ElasticGroup<T, D>* data       (void)               noexcept;
        ElasticGroup<T, D>& operator[] (size_t)             noexcept;
