...
namespace gd{
    ...
    auto List<T>::ref(void) const noexcept -> typename std::enable_if_t<(N==0), const T*>
    {
        return _elements;
    }
    ...
    template <typename T>
    const T* List<T>::ref(uint16_t) const noexcept
    {
        return _elements;
    }
    ...
    template <typename T>
    const T* List<T>::spec(void) const noexcept
    { 
        return _elements;
    }
    ...
    template <typename T>
    ConstRange<T> List<T>::crange(void) const noexcept
    { 
        return ConstRange<T>{_elements, _elements+size(_key_)};
    }

    
    template <typename T>
    ConstRange<T> List<T>::crange(const size_t& e_) const noexcept
    { 
    ...
    template <typename T>
    ConstRange<T> List<T>::crange(const size_t& b_, const size_t& e_) const noexcept
    { 
    ...
    template <typename T>
    bool List<T>::moveBack(T&& t_) noexcept
    {
        if(_capacity<_size+1){
            size_t s_ = (_size+1)*2;
            if(void* new_ptr_=std::realloc(_elements, sizeof(T)*s_)){
                ...
                #ifdef GROUPDATA_LIST_WARNING
                warning::failed_to_realloc{std::string{"[gd::List<"}+typeid(T).name()+">::moveBack(T&&)]", _elements, s_};
                #endif
    ...
    // Reference functions
    template <typename T> template<uint16_t N>
    auto ElasticList<T>::ref(void) const noexcept -> typename std::enable_if_t<(N==0), const T*>
    {
        return _elements;
    }
    ...
    template <typename T>
    const T* ElasticList<T>::ref(uint16_t) const noexcept
    {
        return _elements;
    }
    ...
    template <typename T>
    const T* ElasticList<T>::spec(void) const noexcept
    { 
        return _elements;
    }
    ...
    template <typename T>
    ConstRange<T> ElasticList<T>::crange(void) const noexcept
    { 
        return ConstRange<T>{_elements, _elements+size(_key_)};
    }
    ...
    template <typename T>
    ConstRange<T> ElasticList<T>::crange(const size_t& e_) const noexcept
    { 
        return ConstRange<T>{_elements, _elements+e_};
    }
    ...
    template <typename T>
    ConstRange<T> ElasticList<T>::crange(const size_t& b_, const size_t& e_) const noexcept
    { 
        return ConstRange<T>{_elements+b_, _elements+e_};
    }
    ...
    template <typename T>
    bool ElasticList<T>::moveBack(T&& t_) noexcept
    {
        if(_capacity<_size+1){
