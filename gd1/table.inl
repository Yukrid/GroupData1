...
namespace gd{
    ...
    // Access Functions
    template <typename... As>
    void Table<As...>::_operator_equal(const Table<As...>& tb_) const noexcept
    {
        _size    =tb_._size;
        _capacity=tb_._size;
    ...
    template <typename... As>
    void Table<As...>::_operator_equal(Table<As...>&& tb_) const noexcept
    {
        _size    =tb_._size;
        _capacity=tb_._capacity;
    ...
    // Reference functions
    template <typename T, typename... As> template<uint16_t N>
    const auto Table<T, As...>::ref(void) const noexcept
    {
        return _ref(Rank<N>{ });
    }
    ...
    template <typename T, typename... As> template<uint16_t N>
    const auto Table<T, As...>::_ref(Rank<N>) const noexcept
    {
        return _tables._ref(Rank<N-1>{ });
    }
    
    
    
    template <typename T, typename... As>
    const View<T> Table<T, As...>::_ref(Rank<0>) const noexcept
    {
    ...
    template <typename T, typename... As> template <uint16_t N>
    auto Table<T, As...>::_spec(Rank<N>) const noexcept
    -> Tuple<const T*, const As*...>
    {
        return Tuple<const T*, const As*...>{_elements, _tables.spec()};
    }
    

    
    template <typename T, typename... As>
    auto Table<T, As...>::_spec(Rank<1>) const noexcept
    -> Tuple<const T*>
    {
    ...
    template <typename T, typename... As>
    auto Table<T, As...>::spec(void) const noexcept
    ...
    template <typename T, typename... As>
    Table<T, As...>& Table<T, As...>::_operator_equal(const Table<T, As...>& tb_) noexcept
    {
        std::memcpy(_elements, tb_._elements, tb_.size()*sizeof(T));
    ...
    template <typename T, typename... As>
    Table<T, As...>& Table<T, As...>::_operator_equal(Table<T, As...>&& tb_) noexcept
    {
        _elements=tb_._elements;
        tb_._elements=nullptr;
    ...
    {
        for(size_t a=0; a<size(); ++a) tb_.template path<N>()[a]=this->ref<N>()(a);
    ...
    {
        for(size_t a=0; a<size(); ++a) tb_.template path<0>()[a]=this->ref<0>()(a);
    ...
    // Reference functions
    template <typename T, typename... As> template<uint16_t N>
    const auto ElasticTable<T, As...>::ref(void) const noexcept
    {
        return _ref(Rank<N>{ });
    }
    ...
    template <typename T, typename... As> template<uint16_t N>
    const auto ElasticTable<T, As...>::_ref(Rank<N>) const noexcept
    {
        return _tables._ref(Rank<N-1>{ });
    }
    ...
    template <typename T, typename... As>
    const ElasticView<T> ElasticTable<T, As...>::_ref(Rank<0>) const noexcept
    {
    ...
    template <typename T, typename... As> template <uint16_t N>
    auto ElasticTable<T, As...>::_spec(Rank<N>) const noexcept
    -> Tuple<const T*, const As*...>
    {
        return Tuple<const T*, const As*...>{_elements, _tables.spec()};
    }
    ...
    template <typename T, typename... As>
    auto ElasticTable<T, As...>::_spec(Rank<1>) const noexcept
    -> Tuple<const T*>
    {
        return Tuple<const T*>{_elements};
    }
    ...
    template <typename T, typename... As>
    auto ElasticTable<T, As...>::spec(void) const noexcept
    -> decltype(_spec(std::declval<Rank<sizeof...(As)+1>>()))
    {
        return _spec(Rank<sizeof...(As)+1>{ });
    }
